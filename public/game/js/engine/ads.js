...
// on the plain web build (no native SDK available) it falls back to a
// short simulated ad overlay so the reward loop is still fully testable.
//
...
export async function watchRewardedAd(onReward, onClose) {
  ...
  if (AdMob) {
    try {
      ...
      // native path failed (no fill, plugin error, offline) — fall through
      // to the simulated ad so the crate is still reachable.
    }
  }
